...
// Hàm tính số Fibonacci thứ k (làm riêng, không liên quan trực tiếp đến Stack)
long long Fibonacci(int k);

long long FibonacciRecusion(int k);
...
int main()
{
    ...
    printf("\n=========Fibonacci Series=========\n");
    int n;
    printf("Enter n to print first n Fibonacci numbers: ");
    scanf("%d", &n);
    if (n > 0)
    {
        printf("First %d Fibonacci numbers (starting 1, 1): ", n);
        for (int i = 1; i <= n; i++)
        {
...
long long FibonacciRecusion(int k)
{
    ...
    if (k <= 0)
    {
        return 0; // Số Fibonacci thứ 0
    }
    if (k == 1)
    {
        return 1; // Số Fibonacci thứ 1
    }
    if (k == 2)
    {
        return 1; // Số Fibonacci thứ 2
    }
